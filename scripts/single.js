...
function clickedBox(element) {
  if (players.classList.contains("player")) {
    //if player chooses O
    playerSign = "O";
    //adding O inside user clicked box. Had to go with innerHTML over textContent because the later prints everything as a string. it looks funny if anybody wants to test it :D
...
//function to grab the id value assigned to a box after it has been clicked on
function getId(classname) {
  //return id value
  return document.querySelector(".box" + classname).id;
}

//together with getId, checks to see if any three values have the same sign
function checkIdSign(part1, part2, part3, sign) {
  if (getId(part1) == sign && getId(part2) == sign && getId(part3) == sign) {
    return true;
  }
}
//uses checkIdSign to check and see if different combinations of boxes share the same sign, if finds one of the possibilities exists, a winner is called
function selectWinner() {
  if (
    ...
    cpuTurn = false;
    CPU();
    //reveal the results box modal and removes the board after a brief delay
    setTimeout(() => {
...
//same functionality as replay, just one thats available to the player at any point before the games called
restart.onclick = () => {
  window.location.reload();
};
